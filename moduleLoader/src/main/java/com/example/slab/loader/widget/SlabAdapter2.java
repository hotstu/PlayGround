...
public class SlabAdapter2<VH extends RecyclerView.ViewHolder, T> extends RecyclerView.Adapter {
    /**
    ...
    public interface ArrayAdapterDelegate<VH extends RecyclerView.ViewHolder, T> {
        VH onCreateViewHolder(SlabAdapter2<VH, T> adapter, ViewGroup parent, int viewType);

        void onBindViewHolder(SlabAdapter2<VH, T> adapter, VH holder, int position);

        int getItemViewType(SlabAdapter2<VH, T> adapter, int position);

        /**
         * 当设置hasStableIds为true时被调用， 通常id和positon无关时
         **/
        long getItemId(SlabAdapter2<VH, T> adapter, int position);
    ...
    public SlabAdapter2(ArrayAdapterDelegate<VH, T> delegate) {
