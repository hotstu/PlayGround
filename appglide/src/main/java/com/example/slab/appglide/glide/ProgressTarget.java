...
public abstract class ProgressTarget<T, Z> extends WrappingTarget<Z> implements UIProgressListener {
	...
	@Override public void onProgress(long bytesRead, long expectedLength) {
		System.out.printf(": %d/%d = %.2f%%%n", bytesRead, expectedLength, (100f * bytesRead) / expectedLength);
		if (ignoreProgress) {
			return;
		}
	...
	private void start() {
		MyOkHttpGlideModule.expect(toUrlString(model), this);
	...
	private void cleanup() {
		...
		onDelivered();
		MyOkHttpGlideModule.forget(toUrlString(model));
		this.model = null;
	}
