...
public class MyBottomSheetBehavior<V extends View> extends CoordinatorLayout.Behavior<V> {
    ...
    public MyBottomSheetBehavior(Context context, AttributeSet attrs) {
        ...
        setSkipCollapsed(a.getBoolean(R.styleable.BottomSheetBehavior_Layout_behavior_skipCollapsed,
                false));
        a.recycle();
        a = context.obtainStyledAttributes(attrs, com.yifeng.mdstudysamples.R.styleable.MyBottomSheetBehavior);
        setShrinkHeight(a.getDimensionPixelSize(com.yifeng.mdstudysamples.R.styleable.MyBottomSheetBehavior_behavior_shrinkHeight, 0));
        a.recycle();
    ...
    public boolean onLayoutChild(CoordinatorLayout parent, V child, int layoutDirection) {
        ...
        mParentHeight = parent.getHeight() - mShrinkHeight;
        int peekHeight;
        if (mPeekHeightAuto) {
            if (mPeekHeightMin == 0) {
...
//        return (nestedScrollAxes & ViewCompat.SCROLL_AXIS_VERTICAL) != 0;
    ...
    /**
     * set the height of the bottom sheet when it is hidden
     *
     * @param shrinkHeight
     */
    public final void setShrinkHeight(int shrinkHeight) {
        if (mShrinkHeight == shrinkHeight) {
            return;
        }
        mShrinkHeight = shrinkHeight;
        if ( mState == STATE_HIDDEN && mViewRef != null) {
            V view = mViewRef.get();
            if (view != null) {
                view.requestLayout();
            }
    ...
    /**
     * Gets the height of the bottom sheet when it is hidden.
     * @return
     */
    public final int getShrinkHeight() {
        return mShrinkHeight;
    }
    ...
    void dispatchOnSlide(int top) {
        View bottomSheet = mViewRef.get();
        if (bottomSheet != null) {
            paddingNestedScroll(bottomSheet);
        }
    ...
    /**
     *  add padding at bottom of NestedScrollview
     * @param bottomSheet
     */
    private void paddingNestedScroll(View bottomSheet) {
        int offset = bottomSheet.getTop() - mMinOffset;
        View view = mNestedScrollingChildRef.get();
        if (view != null && offset >= 0) {
            //TODO keep original padding
            view.setPadding(0,0,0, offset);
        }
    }
